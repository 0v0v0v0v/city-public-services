...
const resourceCards = [
  ...
  {
    title: '点位管理',
    description: '维护点位详情、分类归属、推荐标记与草稿/发布状态，是后台的核心内容库。',
    to: '/admin/points',
  },
  {
    title: '资讯管理',
    description: '维护标题、摘要、正文与发布状态，用于首页动态和资讯列表展示。',
    to: '/admin/news',
  },
]
...
<template>
  <AdminShell
    title="便民资源维护台"
    description="把后台收敛为清晰的资源管理入口。先看总览，再进入分类、点位和资讯三个模块做增删查改。"
  >
    ...
    <template v-else>
      <section class="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <div class="grid gap-5 md:grid-cols-3">
          ...
          <StatCard eyebrow="资讯数量" :value="String(metrics?.news_count || 0)" description="资讯承载公告与动态更新。" tone="amber" />
        </div>
        <FeaturePanel
          badge="管理建议"
          title="先让每类数据可维护，再追求数据规模。"
        ...
        >
          <ul class="space-y-2 text-sm leading-7 text-slate-300">
            ...
            <li>点位和资讯建议先以草稿保存，核对后再发布到前台。</li>
            <li>若分类下已有点位，系统会阻止直接删除，避免误删内容。</li>
          </ul>
        </FeaturePanel>
      </section>
